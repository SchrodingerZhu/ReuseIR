    ...
    func.func @rc_release(%x : i64, %y : i64) -> !fsrc {
        %1 = reuse_ir.region.run {
          ^bb(%ctx : !reuse_ir.region_ctx):
             ...
             %urc_ = reuse_ir.rc.create value(%x) region(%ctx) : (i64, !reuse_ir.region_ctx) -> !urc
             %urc = reuse_ir.nullable.nonnull (%urc_ : !urc) : !nullable
             %struct = reuse_ir.composite.assemble (%urc, %null) : (!nullable, !nullable) -> !struct
             %usrc = reuse_ir.rc.create value(%struct) region(%ctx) : (!struct, !reuse_ir.region_ctx) -> !usrc
             ...
             %urc2_ = reuse_ir.rc.create value(%y) region(%ctx) : (i64, !reuse_ir.region_ctx) -> !urc
